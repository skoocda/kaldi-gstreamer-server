...
class SQSClient(WebSocketClient):
    ...
    def opened(self):
        #print "[CLIENT] Queueueue Socket open"
        def send_data_to_ws():
            f = open(self.fn, "rb")
...
def main():
    ...
    parser.add_argument('-u', '--uri', default="ws://54.221.51.233/client/ws/speech", dest="uri", help="Server websocket URI")
    parser.add_argument('-r', '--rate', default=96000, dest="rate", type=int, help="Rate in bytes/sec at which audio should be sent to the server. NB! For raw 16-bit audio it must be 2*samplerate!")
    ...
    ws = SQSClient(args.audiofile, args.uri + '?%s' % (urllib.urlencode([("content-type", content_type)])), byterate=args.rate,
